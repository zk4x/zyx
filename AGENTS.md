...
- **Vendor kernels**: a vendor can contribute a kernel for a subgraph written either in any backend language (CUDA, SPIR-V, ...) **or** in the zyx kernel IR API. Kernels built via the kernel IR API go through all the usual optimizations, including autotuning. Vendor kernels **compete alongside** the heuristics-based fusion variants in the egraph — they are extracted by the same timing-driven path, so the fastest measured variant (heuristic or vendor) wins.
 - **Custom kernels BYPASS the egraph AND autotune.** A hand-written kernel (builder API `Kernel::new` → `CompiledKernel::compile` in `kernel/custom.rs`) is a **pre-fused block** — linear SSA, fully mutable, no egraph inside it and no egraph rewriting it. `CompiledKernel::compile` does only `linearize` → `instruction_schedule` → `constant_folding` → `dead_code_elimination` → `verify` → `device.compile` (PTX/codegen); it does **NOT** run `BeamSearch`, `default_optimizations`, `default_epilogue`, or any cost/timing search. The user has full control: a custom kernel runs exactly as written (only those 3 passes + verify). If the user wants autotuning on a custom kernel they must call `BeamSearch::run_` themselves — nothing is automatic. The egraph also treats custom kernels as opaque fused units and cannot split, re-fuse, or optimize across their boundary. (Future possibility, not now: egraph-side auto-epilogue fusion with custom kernels.)
...
- **REVIEW-THEN-LAUNCH for Tenstorrent kernels.** A hung TT kernel wedges the board (see above), so a launch is never the first look at a new or changed kernel. Before launching, capture the generated sources (`ZYX_DEBUG=16`) and compare reader/compute/writer loop structure, CB acquire/wait/push/pop order, and init sequence against the official tt-metal examples (`tt_metal/programming_examples/`, `ttnn/.../kernels/`). Re-review ALL THREE section kernels after ANY codegen change, even one confined to a single section: traffic correspondence is cross-section, so a compute-side change can break the reader/writer contract. The review must explicitly verify per-CB producer/consumer correspondence (push level and count in the producer vs wait/pop level and count in the consumer). Launch only on structural match. A mismatch is a codegen bug — fix it host-side and re-capture, do not "just try it on the board". "Done" for codegen work means the hardware test passes, never the dump-only gate. A launch is FORBIDDEN unless the kernels are KNOWN valid, which means both: (1) every codegen validity check passes at compile (sections, CB count/pages, push/pop totals, unpacker inputs trafficked, DRAM untouched from compute) — a check failure is a codegen bug, never bypassed; (2) the eye review is written out in chat covering all three section kernels vs the official examples plus the per-CB producer/consumer correspondence table. A partial review is not a review: on 2026-09-12 a compute-only review launched a kernel whose reader/compute traffic mismatched, hung the board, and left it spinning at full fans after the host was killed. A dry run (`ZYX_TT_DUMP_ONLY=1`) passing proves NOTHING except that codegen avoided `todo!()`: host-side compile and `device.compile` do not JIT the C++, so undeclared identifiers (op-id vs register naming, e.g. `r10`/`r17`), type mismatches, and `-Werror`-class warnings all pass silently. The dry run exists for ONE purpose: the agent reads all three emitted sources line by line and validates them as compilable tt-metal — every identifier declared before use, every loop closed, every CB declared, arg counts matching the runtime config — against the official examples, written out in chat. "Green, next" without that review is the failure.
