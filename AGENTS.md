...
- **NEVER DESIGN. NOT EVER.** Designing is the user's job, exclusively. The moment you catch yourself weighing options, drafting an approach, planning an implementation strategy, or choosing between alternatives — even silently, even "just to propose it" — STOP and ask the user instead. Presenting option lists, trade-off analyses, or "here's what I'd do" is designing and is FORBIDDEN. Ask what the user wants done; they supply the design, you type it in. Repeated violations of this rule are the single most common failure mode in this repo's history. When asked "what do you recommend/think", the ONLY correct response is to turn the question back: state what you need to know and ask the user to decide.
...
- **Custom kernels BYPASS the egraph.** A hand-written kernel (builder API, `Kernel::new` in `kernel/custom.rs`) is a **pre-fused block** — linear SSA, fully mutable, no egraph inside it and no egraph rewriting it. The egraph treats it as an opaque fused unit and cannot split, re-fuse, or optimize across its boundary. The layering is strict: **egraph decides WHAT to fuse → emits `Kernel`s → autotune (kernel-level, egraph-agnostic: seed → opt search → epilogue → cost model) decides HOW each kernel runs.** The only coupling is the measured timing handed back for extraction. (Future possibility, not now: egraph-side auto-epilogue fusion with custom kernels.)
...
The autotuner runs **per kernel** and is **egraph-agnostic** — it knows nothing about where a kernel came from (egraph fusion variant or a hand-written custom kernel); its world is linear SSA. Its inputs are: a **seed** (iterator over kernels to start from), a **search** over optimization passes (closure), an **epilogue** (closure run on `&mut Kernel` before measurement, e.g. cleanup/normalize), and a **cost model** (closure) — all four are swappable. For a given kernel it generates **thousands of variants** by running the different optimizations and the different **configurations** of those optimizations (each `config()` returns a `#variants` count — see below). Variant selection uses a **combination of a cost model and measured timings**: the cost model is **regression / neural-net based** (learned, not a hand-written analytic estimate), and the **user configures the ratio** between how many variants are scored by the cost model versus actually launched and timed. The egraph generates a **few fusion variants** of each subgraph; **each fusion variant is autotuned and launched (timed) at least once** — there is no path that picks a fusion variant purely on the cost model without a real launch. The measured timing is what the egraph consumes to extract the fastest path. Inside a single kernel, the autotuner explores many **variants (tilings, vectorization, coarsening, ...)** — these are NOT all launched; they are ranked by the trainable cost model (`base_cost`), with only selected candidates measured.
