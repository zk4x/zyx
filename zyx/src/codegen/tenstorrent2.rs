use crate::{
    DType, Map,
    error::BackendError,
    kernel::{IDX_T, Kernel, MMADType, MemLayout, Op, OpId, RangeKind},
};
...
impl Kernel {
    ...
    pub(crate) fn generate_tenstorrent(&self, debug_asm: bool) -> Result<(String, String, String), BackendError> {
        let cb_map: Map<OpId, CBId> = Map::default();
        let reader_params: Vec<CBId> = Vec::new();
        let compute_params: Vec<CBId> = Vec::new();
        let writer_params: Vec<CBId> = Vec::new();

        let mut reader_src = String::new();
        let mut compute_src = String::new();
        let mut writer_src = String::new();

        Ok((reader_src, compute_src, writer_src))
    }
