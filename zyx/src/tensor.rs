    ...
    T: Copy,
{
    ...
    fn zip(self) -> Self::IntoIter { self.0.into_iter().zip(self.1) }
}
    ...
    T: Copy,
{
    ...
    fn zip(self) -> Self::IntoIter { self.0.into_iter().zip(self.1).zip(self.2).map(|((a, b), c)| (a, b, c)) }
}
...
impl Tensor {
    ...
    pub fn load_gguf(
        ...
        let metadata_kv_count = usize::try_from(u64::from_le_bytes(metadata_kv_count))
            .map_err(|e| ZyxError::ParseError(format!("Failed to parse tensor count in GGUF file. {e}").into()))?;
    ...
    fn load_safetensors(path: impl AsRef<Path>) -> Result<Map<String, Tensor>, ZyxError> {
        ...
        let n = usize::try_from(u64::from_le_bytes(header_len))
            .map_err(|e| ZyxError::ParseError(format!("Failed to parse header len in safetensors file. {e}").into()))?;
                        ...
                        shape = text
                            ...
                            .map(|d| {
                                d.parse::<usize>().map_err(|err| {
                                    ZyxError::ParseError(format!("Cannot parse safetensors shape: {err}").into())
                                })
                            })
                        ...
                        let offsets = text
                            ...
                            .map(|offset| {
                                offset.parse::<usize>().map_err(|err| {
                                    ZyxError::ParseError(format!("Could not parse safetensors offset: {err}").into())
                                })
                            })
...
impl IntoIndex for Vec<Range<isize>> {
    fn into_index(self) -> impl Iterator<Item = Range<isize>> { self.into_iter() }
}

impl<I: IntoRange> IntoIndex for &[I] {
    fn into_index(self) -> impl Iterator<Item = Range<isize>> { self.iter().cloned().map(IntoRange::into_range) }
}

impl<I0: IntoRange> IntoIndex for I0 {
    fn into_index(self) -> impl Iterator<Item = Range<isize>> { [self.into_range()].into_iter() }
}

impl<I0: IntoRange, I1: IntoRange> IntoIndex for (I0, I1) {
    fn into_index(self) -> impl Iterator<Item = Range<isize>> { [self.0.into_range(), self.1.into_range()].into_iter() }
}
