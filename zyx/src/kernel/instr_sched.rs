...
impl Kernel {
    ...
    pub fn instruction_schedule(&mut self) {
        ...
        while !op_id.is_null() {
            ...
            match self.at(op_id) {
                Op::Define { scope: MemScope::Global | MemScope::Variable, ro: true, .. } => global_ro.push(op_id),
                Op::Define { scope: MemScope::Global | MemScope::Variable, ro: false, .. } => global_rw.push(op_id),
