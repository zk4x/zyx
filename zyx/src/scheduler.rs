...
use crate::{
    backend::{BufferId, Device, DeviceId, DeviceInfo, MemoryPool, MemoryPoolId},
    graph::Graph,
    index_map::Id,
    ir::IRKernel,
    kernel::Kernel,
    optimizer::{KernelOptimization, KernelOptimizer},
    tensor::TensorId,
    view::View,
    ZyxError,
};
            ...
            for output in kernel.outputs() {
                ...
                if !tensor_buffer_map.contains_key(&key) {
                    ...
                    sched_graph.push(SchedulerOp::Allocate {
                        tensor_id: output,
                        memory_pool_id,
                        bytes: shape.iter().product::<usize>() * graph.dtype(output).byte_size(),
                        view: view.clone(),
                    });
...
pub(super) fn compile_cached(
    ...
    if program_id.is_none() {
        ...
        kernel_cache.insert(ir_kernel, (device_id, program_id.unwrap()));
    }
    Ok((
        program_id.unwrap(),
        ir_args
