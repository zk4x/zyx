...
use crate::{backend::Device, graph::Graph, ir::IRKernel, kernel::{Kernel, Op, TId}, node::Node, optimizer::Optimizer, tensor::TensorId, view::View, DebugMask, ZyxError};
...
pub(super) fn realize_graph(graph: &Graph, order: &[TensorId], to_eval: &BTreeSet<TensorId>, devices: &mut [Device], optimizer: &mut Optimizer, search_iters: usize, debug: DebugMask) -> Result<(), ZyxError> {
    ...
    for nid in order.iter().copied() {
        ...
        if kernel_outputs[kid].is_empty() {
            ...
            if device.is_cached(&kernel) {
                ...
                let optimization = optimizer.get_optimization(&kernel, device_info, search_iters);
                let optimized_kernel = kernel.optimize(optimization);
                let ir_kernel = IRKernel::new(optimized_kernel);
                device.compile(kernel, &ir_kernel, true);
            }
