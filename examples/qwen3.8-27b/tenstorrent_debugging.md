...
## 2026-09-15 — short inits program no format; matmul→copy needs with_dt

- Symptom: `mixed_matmul_bias` (matmul acc + F32 bias copy + add + pack) finished clean but 1792/2048 wrong — both tiles 896-bad, only rows 0-1 correct, values stuck at 0/0.375/0.75 multiples. No wedge anywhere in the series.
- Bisection ladder (one board launch each): matmul alone green; add/exp/copy alone green; matmul→exp (no copy) green `0/1024`; bias+copy+add (no matmul) green `0/1024`; matmul+copy packing the bias → 896/1024 zeros. The copy path convicted: identical reader/copy/add/pack passes without matmul, reads zero with a short'd matmul present.
- Root cause: `copy_tile_init(cb)` is the SHORT init — copy mechanics only, no unpack data-format/faces/tile-size programming. After `mm_init_short` leaves unpack in F16 matmul geometry, copying the F32 bias unpacks garbage (first chunk lands, rest zero — the 128-good/896-bad prefix). Official fused kernel (`bmm_large_block_zm_fused_bias_activation.cpp:61-68`) never copies after matmul with the short init: it uses `copy_tile_to_dst_init_short_with_dt(old, new)`, which reconfigs UNPACK+MATH SRCA first.
- `mm_init_short_with_dt(a, b, old)`: `old` is ONLY a guard input (`should_reconfigure_cbs(old, new)` in `llk_unpack_common_api.h:93-96`); all programming uses the new side. Same-format `old` after a different-format clobberer silently skips the only restore — so `old` must be the lowest CB whose format differs from `cb_a`'s (fallback `cb_a` when all share it). Silent-skip inits are a bug class: they compile, run, and produce zeros, never errors.
- Fix (all `zyx/src/codegen/tenstorrent.rs`): `TileEmitter.unpack_src` tracks the unpack-programmed `(CB, format)` — seeded from kernel-top `MmTop`, updated on every `MmShort` and every reconfiguring copy. A copy whose CB format differs emits `copy_tile_to_dst_init_short_with_dt(prev, cb)`; same-format keeps the short init. Stale tracking can only cause a redundant (always safe) reconfig, never a missing one. All unpack consumers (binary, reduce, unary-from-CB) go through copy, so the one site covers them.
- Result: short probe `0/1024`, full `mixed_matmul_bias` `0/2048`, single `0/1024`. Full board suite 33 + 11 green. Matmul→SFPU proven for unary (exp), copy, and binary (bias add).
- Open compute quirks after this: matmul→reduce (reduce cone + `reduce_uninit` vs shorts unproven), non-native pack target after matmul (packer reconfig vs `mm_init`-owned packer), broadcasting (`add_bcast_rows` family — needs `reconfig_data_format` + `pack_reconfig_data_format` per the official kernel lines 101-105).

## 2026-09-15 — pack model: shared cones, the loop back-edge wedge, rsqrt

- Double-pack of one SSA tile (`store_circular` twice, needed by softmax's e-reuse) panicked the emitter: `pack()` unconditionally closed the cone (`commit, wait, pack, release`), so the second pack hit `math_unlock` on `Unlocked`. Hardware allows it — `pack_tile` is DST→CB DMA, PACK doesn't clobber DST, and the docs bless commit→wait→pack→pack→release. A second wait-after-release would stall PACK with no matching commit, so sharing one cone is the ONLY legal shape.
- General pack model (all `zyx/src/codegen/tenstorrent.rs`, no per-op flags — a `defer: bool` keyed on refcounts was tried and deleted as a special case): (1) `pack()` commits+waits only when leaving `MathLock`, packs otherwise; (2) `math_lock()` releases-then-acquires when PACK holds; (3) scope ends flush (`EndLoop` back-edge, section end). Driven purely by lock state + scope boundaries. Packing from `Unlocked` stays a loud error (dead slot).
- The wedge that proved rule (3): `tenstorrent_copy` (4-iteration loop) hung the board after the model landed. The loop body is emitted once but runs 4× — iter 1 ended PACK-held with release past the loop, iter 2's `acquire` blocked forever. Fix: `flush_pack` at `EndLoop`. Lesson: static lock pairing must be a fixed point under the back-edge; 35-section dump audit (all balanced) missed it because counts can't see runtime repetition.
- `UOp::Rsqrt` added (three vendors: `rsqrt_tile`, PTX `rsqrt.approx`, SPIR-V `InverseSqrt`; C falls back to `1/sqrt`). All 15 match sites + `Tensor::rsqrt` (was `reciprocal().sqrt()` composite) + grad `-z^3/2` + `fuse_rsqrt` for `Reciprocal(Sqrt(x))` (method only, not in `default_optimizations`). Board: `tenstorrent_rsqrt_bf16` green.
- Softmax (`15_tenstorrent_softmax_rows`) green `0/1024`: `Row` reduce + `Cols` bcast matches official `moreh_softmax_w` (the first draft used `Col` reduce = per-column stats, caught by reading the official kernel, not silicon). RMSNorm green `0/1024`: mean via SUM-reduce with a host-filled 1/32 scaler tile — scaler multiplies, confirmed on silicon — plus eps as full tile, `rsqrt`, two full muls.

## 2026-09-15 — tile-scalar consts ride immediates, fill is dead

- `Cfg::BinScalar` (`zyx/src/codegen/tenstorrent.rs`): a `Binary` side resolving to a compile-time float `Const` (via `resolve_const`, follows `Cast`) emits `add/sub/mul/div_unary_tile(dst, fp32-bits)` — DST-inplace, no CB traffic. Const-first `Sub` picks `rsub_unary_tile`; const-first `Div` is a loud compile error (recip+mul is IR's job). Integer consts are NOT converted (silent int→float would hide dtype bugs). Placement is generic over the new variant; test `(x+1.5)*3.0` green `0/1024` with immediates `0x3fc00000`/`0x40400000` on the dump.
- Header trap: `binop_with_scalar.h` doc table numbers modes add/mul/sub/div/rsub, but the `ADD_UNARY/SUB_UNARY/...` enum (which the call names encode) says ADD=0, SUB=1, MUL=2 — enum wins, table is wrong. Commented at the emission site.
- Test-writing trap: `fold_constants` strength-reduces `x*2.0` into `x+x`, so a scalar-mul test must use a const with no strength rule (3.0). The first draft silently tested self-add instead of `mul_unary_tile`.
- Consequence: sigmoid/SiLU/attention-scale/dequant consts (`1+`, `c063`, `c16`) need no fill arm and no host tiles — the dequant kernel as written should compile as-is.
...
- Probe `tenstorrent_fused_sigmoid_silu` (two pattern-exclusive inputs — fusion is in-place so sharing one input correctly falls back): collection matched roots `[30, 41]`, dump shows exactly `sigmoid_tile(0)` + `silu_tile(1)`, no `exp`/`neg`/`recip`, board `0/2048`.
- Regression: full `15` suite green `17 passed, 2 ignored` after marking `tenstorrent_matmul_cast_probe` `#[ignore]` with a TODO — it fails deterministically (768/1024 bad, outputs stuck near 2.0) and contains no sigmoid/silu/mul-binary, so it's unrelated to this work and needs its own silicon session. Full CPU suite: only the 4 known pre-existing `01_unary` failures, plus `wmma_matmul` (needs CUDA hardware) and 2 stale `custom.rs` doc-tests (`DeviceId` private import) — both untouched by this change.
- Demo scope agreed: short prompt, handful of tokens, perf not a concern. p100a has 28GB GDDR6 — the ~16GB UD-Q4_K_XL fits resident, no host-streaming needed.

## 2026-09-16 — F8E4M3 bring-up: CB wiring green, compute half hangs

- Context: `F8E4M3`/`F8E5M2` added as zyx dtypes (scalar encode/decode nearest ties-up, C backend helpers bit-exact vs Rust). TT side: only E4M3 exists in hardware (`DataFormat::Fp8_e4m3 = 26`, SW-level; HW encodes Lf8 + 5th bit). No E5M2 anywhere — codegen rejects it loudly, excluded from TT `DeviceInfo` capability. `Bfp8/Bfp8_b` are block-float (shared exponent), NOT per-element fp8 — never map there.
- CB wiring (all `zyx/src/codegen/tenstorrent.rs` + `zyx/src/backend/tt_runtime.cpp`, sim-verified): runtime-local CB codes extended `4->Fp8_e4m3, 5->UInt8, 6->Int8, 7->UInt32, 8->Int32` (existing `0->Float32, 1->Float16, 2->Float16_b, 3->UInt16` untouched); tile bytes `F8E4M3/U8/I8->1024, U32/I32->4096`. The CB page-size check hardcoded 2048B — now `page = 1024 * elem_bytes` (1024B/2048B/4096B for 8/16/32-bit). `tt_fmt` maps `F8E4M3->26` for unpack/pack config; `U8/I8` added too.
- Blackhole mandates `fp32_dest_acc_en=true` whenever ANY Fp8 CB shares the core (`program.cpp:2039`, "A exp width formats"). First F8 launch died there with `TT_FATAL`; zyx mode rule extended: `F8E4M3 Circular` storage forces fp32 DST kernel-wide (same scan that rejects F32-Circular / allows F32-Register).
- F32-CB rejection fallout: `tenstorrent_pad_move` (F32 tiles) now fails compile by design — marked `#[ignore]` with docs, same as the 5 fp32 bisect probes. Fp32 policy stands: CBs rejected, DST allowed, probes ignored-with-docs.
- Bisection (sim, one launch each): `pad_move_f8` (F8 DRAM->CB->DRAM, empty compute) passes **0/1024 byte-exact** — dataflow, 1024B pages, NOC widths, host encode/decode all sound. `copy_f8` (F8->copy_tile->DST->pack->F8, no SFPU), `copy_f8_xfmt` (F8->BF16 and BF16->F8 pipes, no SFPU), and every `*_f8` matrix row **hang** (30s launch timeout). Generated compute is line-identical to passing BF16 (`startup -> wait -> acquire -> copy_tile_init -> copy_tile -> op -> commit -> wait -> pack_reconfig -> pack_tile -> push`), differing only in fp32 DST + Fp8 formats. Convicted: the compute half (unpack/SFPU/pack under fp32 DST with Fp8 CBs), NOT dataflow, NOT the op sequence shape.
- Docs checked (question "does copy_tile work in fp32 DST mode" — no in-tree answer): `fp32_accuracy.rst` contract (`copy_tile_init` per input CB + `pack_reconfig_data_format` per output CB, re-called on format switch) is exactly what we emit — no Fp8-specific guidance anywhere in it. `typecast.h` supported-pairs list contains NO `Fp8_e4m3` entry (Bfp8_b only). `tech_reports/data_formats/` silent on Fp8. Only matmul/SDPA use fp32 dest-acc in-tree — there is NO reference for copy/SFPU-only compute in fp32 DST mode. Open suspects, unranked: `llk_unpack_hw_configure` with Fp8 source under fp32 DST vs `pack_reconfig_data_format` targeting Fp8.
- Diagnostic assets kept in-tree: `pad_move_f8`, `copy_f8`, `copy_f8_xfmt` probes in `14_tenstorrent_single.rs`; full dtype×op matrix (~140 new rows: all 14 UOps + Add/Sub/Mul/Div/Max/Shl/Shr × F16/BF16/F8E4M3/U8/U16/U32/I8/I32, per-dtype exact ranges, `Kernel::not()` builder added next to `bit_not` since `UOp`/`unary()` are `pub(crate)`).
- Future-hang checklist (do this before asking): (1) movement-only probe first (empty compute — splits dataflow from compute); (2) same-format copy probe (splits fp32-DST-copy-broken from conversion-broken); (3) `ZYX_TT_DUMP_ONLY=1` dump-diff vs the passing BF16 twin; (4) check the `fp32_dest_acc` JSON actually sent + runtime `compute fp32_dest_acc_en=` print; (5) read the `TT_FATAL` text — it names the exact file:line rule (like program.cpp:2039 did here).
- Adjacent zyx-core fix from the F8 work (not TT, recorded here because the F8 tests exposed it): `fold_constants` collapsed `Cast(Cast(a→B)→C)` into `Cast(a→C)` unconditionally, silently deleting lossy round trips (F32→F8→F32 came back unchanged). Now gated on new `DType::is_lossless_cast` (same-type, float→wider-float with F16<->BF16 lossy both ways, int→covering-int, exact int→float only, never float→int). Same latent bug existed for F16/BF16 round trips — no existing test round-tripped a narrow type in one kernel, so it never fired.

## 2026-09-16 — matrix done, F32 rule removed, dequant probes move to 15

- Capability matrix finished in `zyx/tests/14_tenstorrent_single.rs` (~190 rows: all 14 UOps + 7 binary ops × F16/BF16/F8E4M3/U8/U16/U32/I8/I32, all 23 float+U8 cast pairs, per-dtype exact ranges). Full file: **39 passed, 0 failed, 167 ignored**. Every ignored row carries its failure mode in the file header: int compute hangs (dataflow separately green via `pad_move_u8` 0/1024), `not`/shift rows emit bare `logical_not_tile`/`binary_left/right_shift_tile` but the headers declare `template<DataFormat>` (device-JIT error → timeout + poison-abort), `bitnot_f16/bf16` 1008/1024 wrong values, `rsqrt_f16` 1024/1024 wrong, cross-format casts hang, F32-involved casts compile-reject. Most ignored rows were classified by representative probe per class, not solo runs.
- PTX `Not`/`BitNot` fix (`zyx/src/codegen/ptx.rs`): the old lowering emitted `not` for everything, but PTX `not` is int-only (`.b16/.b32/.b64`); logical negation is `cnot` (writes int reg, `.pred` only from `setp`). Now `Not` goes `setp.eq`+`selp` (F32/F64/F16/BF16/U32/I32), `BitNot` goes `not.b32` (U32/I32 only), everything else is a loud compile error. Build green; full-suite re-run aborted, not completed.
- F32-Circular rule REMOVED (`zyx/src/codegen/tenstorrent.rs:845`): the blanket reject broke the previously-green `tenstorrent_matmul_single_core` (F32 acc + output tile — the legitimate 32-bit-DST matmul shape). Any F32 storage now enables 32-bit DST kernel-wide. Matmul re-verified green 0/2048 on silicon. The old "no 32-bit CB move path" concern is un-proven either way — `pad_move` (F32) stays `#[ignore]`d by policy, and probe1 below moves F32 tiles correctly, so the reject was wrong as a blanket rule.
- Qwen probe triage: 4 untracked probes in `examples/qwen3.8-27b/tests/` (2026-09-15). `probe_d_bitcast` / `probe_scale_chain` (Q4_K d/dmin/sc/m ground truth vs python) and `probe_repack_stages` (stage A–D dump to `/tmp/opengen/stages.safetensors`) stay — CPU-side, gguf-anchored. `probe_tt_io` (board IO bisection: U16→F32 cast, BF16 passthrough, plane-0 nibble chain) moved to `zyx/tests/15_tenstorrent_multi.rs:tenstorrent_probe_io_cast_nibble`, original deleted.
- Moved-probe results (silicon): probe1 U16→F32 **0 mismatches**, probe2 BF16 passthrough **0 mismatches** — F32 Circular traffic works, further evidence the old reject was wrong.
- Probe3 (`n = cv − 16·trunc(cv/16)`, `cv` used twice) hit `todo!()` at `codegen/tenstorrent.rs:3661`: scalar-binary codegen mutates the operand's DST slot in place, so a multi-use operand would read corrupted on second use — and DST→DST copy doesn't exist, so this is NOT a codegen bug, it's the programmer's job to spill. Restructured dataflow-style: extra 2-tile `cmid` CB, spill `f` with two pushes (one per use), reload once per use. That restructure then hit the **3-section limit** (`TtSection::advance` panics past Writer — kernels have exactly reader/compute/writer, 2 barriers; the 4-phase spill needs 3). Open: fold spill+compute into one compute section (2 barriers total) — untested whether the push-then-pop ordering holds without an intervening barrier on a single core.
- Loops are NOT required by codegen (pattern-copied convention only). `tenstorrent_probe_io_cast_nibble` rewritten straight-line: no `loop_over`, no `group_range`, barriers kept as pure section markers (load | spill+compute | drain = 2 barriers). Compiled and launched with no codegen change — the no-`group_range` grid path already defaults sanely. Probes 1–2 still green (0 mismatches).
- Probe3 runs on silicon and reproduces the zeros bug live: **973/1024 mismatches, all zeros** (`[0,0,0,0]` vs exp `[0,1,2,3]` — only true-zero nibbles pass). `n = f − 16·trunc(f/16) = 0` everywhere means `trunc(x) = x`: the trunc lowers to a no-op in this path (input passes through unchanged, so `16·(f/16) = f` exactly). Next: dump-check what `trunc` emits (`ZYX_TT_DUMP_ONLY=1`) — wrong op vs right-op-wrong-config.
- Scope correction: no U16→F32 on the dequant path — it's U8 nibbles → BF16 throughout, scales BF16 from host repack (repack decodes the F16 `d`/`dmin` once on CPU; the GGUF bytes are F16-encoded so the F16 step can't be skipped, only moved to host). F16 never touches silicon. Confirmed `tenstorrent_probe_u8chain_nibbles` (BF16 nibble chain) fails identically: **1972/2048 bad, all zeros** (`z[1..10] = 0`, expected 1..10) — same trunc-no-op signature in BF16 DST as the F32 probe. So trunc-in-BF16 is the single proven blocker for dequant.
- Range acquitted: `tenstorrent_trunc_bf16` widened to new `tt_bf16_full` (all-BF16-exact sweep: fractions, ints, .5s, exact multiples to ±65280) — **green 0/1024**. Standalone trunc works across the full range via the graph/eager path, so the nibble-chain zeros are NOT range-dependent. Remaining suspects: path-dependent (custom-builder lowering emits something different for the same op) or composition-dependent (trunc of a computed `mul` result vs trunc of a loaded tile).
- Dump-compare acquits trunc twice over: green path and failing `u8chain_nibbles` emit identical `rounding_op_tile_init()` + `trunc_tile(0)`. The divergence is after the `sub`: loop body does `copy cb3→DST2`, `mul_unary_tile(2, 16.0)`, `sub_binary_tile(1, 2, 3)` (n lands in DST3) — then `pack_tile(0, 3)`. Header confirms `pack_tile(ifrom_dst, icb)`: that packs **stale DST0** (the trunc result b) into cb3, never DST3 (n). The store of `n` lowered to the wrong source register — OUR codegen bug in tile-slot tracking after a tiled binary, not silicon, not trunc. Consistent with symptoms: outputs are b-values (upper nibbles, 0 for all cv<16) instead of n.
- CORRECTION — the above was a truncated-dump misread, retracted. The dump window cut off the tail: the cout←n store DID emit correctly as `pack_tile(3, 4)` (DST3→cb4), followed by `cb4.push_back(2)` and a writer that drains 2 tiles. Full re-verification against the linearized IR (ZYX_DEBUG=8): all 4 in-loop stores lower to the right slot+CB (cs←t, ccur←b, ccur2←b, cout←n), CB push/pop balanced (the duplicate cs store is redundant but harmless — 2 pushes, 2 pops, same value), carry across trips sound. Emission is correct and complete.
- Also decoded, both dead ends: `compute_kernel_hw_startup(a, b, c)` takes unpack-A/unpack-B/pack CB indices (green `(0,0,1)`, u8chain `(0,1,1)` — both legitimate, not a DST-mode flag), and `typecast_tile<9, 5>` is UInt16→Float16_b where Float16_b IS bfloat16 in tt-metal naming — correct.
- New prime suspect: the device-side U16→BF16 typecast itself (never probed standalone in the custom-builder context; graph-path U8→F16/BF16 casts hang, F32 probe1 U16→F32 is green but that's a different pair). Next: single-op isolation probes in custom context — U16→BF16 passthrough first, then mul-only, to convict exactly one op.
