...
/// accumulate. `R` spans `Mt = R/32` tile rows.
///
/// Tilized DRAM on all sides; B stored KN-tilized (`[Kt, Nt]` tiles).
/// Triple loop mt/nt/kt: per output tile, stream 1 A + 1 B tile per Kt
/// iteration (A is MK-tilized, indexed `mti*Kt + kti`). The running sum
...
/// wait/matmul/pop loop with acquire at the pack (nt) scope. 4 params:
/// a, b, zero tile, out.
pub fn gemm_tt(r: i64, k: i64, n: i64) -> Kernel {
    ...
    // Reader: per output tile row and tile, seed acc with zero, then
    // stream Kt pairs. A is MK-tilized: tile (mti, kti) lives at
    // flat index mti*Kt + kti.
    ...
    // Writer: stream packed output tiles to DRAM in tile-row-major order.
    kernel.loop_over(cmt, |kernel, mti| {
        kernel.loop_over(cnt, |kernel, nti| {
            let mn = kernel.mad(mti, cnt, nti);
            let obase = kernel.mad(mn, c1024, c0);
