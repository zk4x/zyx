...
fn train() -> Result<(), ZyxError> {
    ...
    let dataset: HashMap<String, Tensor> =
        Tensor::load(concat!(env!("CARGO_MANIFEST_DIR"), "/../data/mnist_dataset.safetensors"))?;
    ...
    for step in 0..total_steps {
        let indices = Tensor::randint::<i64>([batch_size as i64], 0..n_train as i64)?;
