...
def _print_dt_rust(dt, feature_names, leaf_biases, indent=0):
    ...
    def print_node(node_id, depth=0):
        ...
        if tree.feature[node_id] == -2:  # leaf
            ...
            print(f"{prefix}if {expr} < {threshold:.6}f {{")
...
def main():
    ...
    # Generate a single Rust function: predict_log_time(raw_params) -> f32
    print(f"\n=== Single Rust prediction function ===")
    print(f"// Ridge features: {len(final_ridge_coefs)}, DT leaves: {len(final_dt_coefs)}")
    print(f"// Total parameters: {len(final_ridge_coefs) + len(final_dt_coefs)}")
    print()
    print(f"fn predict_log_time(")
    print(f"    num_groups: u32, wi_per_group: u32, wi_ops: u32,")
    print(f"    wi_compute_ops: u32, wi_barriers: u32,")
    print(f"    wi_global_load_bits: u32, wi_global_store_bits: u32,")
    print(f"    wi_local_load_bits: u32, wi_local_store_bits: u32,")
    print(f"    wi_peak_reg_bytes: u32, wi_branches: u32,")
    print(f"    wi_global_load_lidx_stride: u32, wi_global_store_lidx_stride: u32,")
    print(f"    wi_local_load_lidx_stride: u32, wi_local_store_lidx_stride: u32,")
    print(f"    warp_size: u32, max_local_threads: u32, max_register_bytes: u32,")
    print(f") -> f32 {{")
    print(f"    // Compute transformed features from raw inputs")
    print(f"    let lng = (num_groups as f32).ln();")
    print(f"    let lwpg = (wi_per_group as f32 + 1.0).ln();")
    print(f"    let lops = (wi_ops as f32).ln();")
    print(f"    let lcop = (wi_compute_ops as f32).ln();")
    print(f"    let lgmem = ((wi_global_load_bits + wi_global_store_bits) as f32 + 1.0).ln();")
    print(f"    let log_inv_threads = (1.0 / (num_groups * wi_per_group) as f32).ln_1p();")
    print(f"    let ci = wi_compute_ops as f32 / ((wi_global_load_bits + wi_global_store_bits) as f32).max(1.0);")
    print(f"    let barr = wi_barriers as f32;")
    print(f"    let wr = wi_per_group as f32 / warp_size as f32;")
    print(f"    let rr = wi_peak_reg_bytes as f32 / max_register_bytes.max(1) as f32;")
    print(f"")
    print(f"    // Build feature vector for Ridge")
    print(f"    let features: [f32; {n_ridge_sel}] = [")
    for i in range(n_ridge_sel):
        name = final_names[i]
        # Generate the Rust expression for this feature
        expr = _feature_to_rust(name)
        print(f"        {expr},  // {name}")
    print(f"    ];")
    print(f"")
    print(f"    // DT leaf bias (inline tree traversal)")
    print(f"    let leaf_bias: f32 = {{")
    _print_dt_rust(dt, FEATURE_NAMES, final_dt_coefs)
    print(f"    }};")
    print(f"")
    print(f"    // Ridge linear combination")
    print(f"    let mut result = {final_intercept:.6f};")
    for i in range(n_ridge_sel):
        std = scaler_final.scale_[i]
        mean = scaler_final.mean_[i]
        coef = final_ridge_coefs[i]
        name = final_names[i]
        print(f"    result += {coef:.6f} * (features[{i}] - {mean:.6f}) / {std:.6f};  // {name}")
    print(f"    result += leaf_bias;")
    print(f"    result")
    print(f"}}")
