    ...
    fn evaluate(
        ...
        for nid in order.iter().copied() {
            ...
            match &nodes[nid.i()] {
                ...
                Node::Add(x, y) => {
                    self.tensors
                        .insert(nid, &self.tensors[x] + &self.tensors[y]);
                }
                Node::Sub(x, y) => {
                    self.tensors
                        .insert(nid, &self.tensors[x] - &self.tensors[y]);
                ...
                Node::Pow(x, y) => {
                    self.tensors
                        .insert(nid, self.tensors[x].pow(&self.tensors[y]));
                }
                Node::Cmplt(x, y) => {
                    let kind = self.tensors[x].kind();
                    self.tensors.insert(
                        nid,
                        self.tensors[x].lt_tensor(&self.tensors[y]).to_kind(kind),
                    );
                }
                Node::Where(..) => {
                    todo!()
                ...
                Node::Expand(x, sh) => {
                    self.tensors
                        .insert(nid, self.tensors[x].expand(sh.vi64(), true));
                }
                Node::Permute(x, axes, ..) => {
                    self.tensors
                        .insert(nid, self.tensors[x].permute(axes.vi64()));
                }
                Node::Pad(x, padding, ..) => {
                    let padding: (Vec<i64>, Vec<i64>) = padding.iter().copied().unzip();
                    let padding: Vec<i64> = padding.0.into_iter().chain(padding.1).collect();
                    self.tensors
                        .insert(nid, self.tensors[x].pad(&padding, &"constant", 0.0));
                }
                Node::Sum(x, axes, ..) => {
                    self.tensors.insert(
                        nid,
                        self.tensors[x].sum_dim_intlist(axes.vi64(), true, None),
                    );
                }
                Node::Max(x, axes, ..) => {
                    self.tensors
                        .insert(nid, self.tensors[x].amax(axes.vi64(), true));
                }
            }
